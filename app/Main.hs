...
main = do
  ...
  void $ forkIO (monitorChargingAndSuspend zone updater)
...
monitorChargingAndSuspend :: TimeZone -> MVar (State -> State) -> IO ()
monitorChargingAndSuspend zone mv = withCreateProcess (proc "udevadm" ["monitor", "-k", "-s", "usb_power_delivery", "-s", "nvme"]) {std_out = CreatePipe} $ \_ (Just hout) _ _ -> do
...
getCharging :: IO Bool
getCharging = not . isPrefixOf "Discharging" <$> readFile "/sys/class/power_supply/BAT0/status"
...
getBattery = Battery <$> getCapacity <*> getCharging
